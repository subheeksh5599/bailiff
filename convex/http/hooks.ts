...
/**
 * The hooks the outside world calls, and the only way evidence gets in.
 *
 * Every route here is fail-closed: with no shared secret configured it refuses
 * rather than accepts, so an unconfigured deployment records nothing instead of
